...
function OpsNav({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return (
    <>
        ...
        {NAV.map((g) => (
          <div key={g.section}>
            <div className="flex items-center gap-1.5 px-2 pb-1.5">
              <span className="h-px w-2.5 bg-gradient-to-r from-emerald-400/60 to-transparent" aria-hidden />
              <span className="text-[9.5px] font-bold uppercase tracking-[0.16em] text-zinc-600">{g.section}</span>
            </div>
                ...
                return (
                  <button key={it.id} onClick={() => { setOpsView(it.id); onNavigate?.() }}
                    className={cn('relative flex min-h-11 w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-[13px] font-medium transition',
                      active
                        ? 'bg-gradient-to-r from-emerald-500/20 to-emerald-500/[0.06] text-emerald-300 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]'
                        : 'text-zinc-400 hover:bg-white/5 hover:text-zinc-200')}>
                    {active && <span className="absolute -left-3 top-1/2 h-5 w-1 -translate-y-1/2 rounded-r-full bg-gradient-to-b from-emerald-300 to-teal-500 shadow-[0_0_12px_rgba(16,185,129,0.8)]" aria-hidden />}
                    <span className={cn('transition', active ? 'text-emerald-300' : 'text-zinc-500')}>{it.icon}</span>
      ...
      <div className="flex items-center gap-2 border-t border-white/[0.06] bg-black/30 p-3">
...
export function OpsApp() {
  ...
  return (
    <div className="flex h-dvh bg-zinc-100 text-zinc-900 sm:h-screen">
      {/* Desktop sidebar */}
      <aside className="relative hidden w-60 shrink-0 flex-col bg-zinc-950 md:flex">
        <div className="pointer-events-none absolute inset-0" aria-hidden>
          <div className="absolute inset-x-0 top-0 h-56 bg-gradient-to-b from-emerald-500/[0.13] to-transparent" />
        </div>
        <div className="relative flex items-center gap-2.5 px-5 pb-4 pt-5">
          <span className="glow-emerald flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-400 to-teal-600"><QrCode className="h-4.5 w-4.5 text-white" /></span>
          <div>
            <div className="text-[13px] font-bold tracking-tight text-white">EasySourcing</div>
            <div className="text-[9.5px] font-bold uppercase tracking-[0.16em] text-emerald-400/80">Operations Portal</div>
          </div>
        </div>
        <OpsNav />
      </aside>
        ...
        <header className="sticky top-0 z-30 flex h-16 shrink-0 items-center justify-between gap-3 border-b border-zinc-200/80 bg-white/80 px-3 backdrop-blur-xl sm:px-4 lg:px-6">
          <div className="flex min-w-0 items-center gap-2.5">
            ...
            <button onClick={() => setSurface('landing')} aria-label="Back to all surfaces" className="hidden h-10 w-10 shrink-0 items-center justify-center rounded-lg transition hover:bg-zinc-100 md:flex">
              <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-emerald-400 to-teal-600 shadow-[0_2px_8px_rgba(16,185,129,0.4)]"><QrCode className="h-4 w-4 text-white" /></span>
            </button>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden items-center gap-2 rounded-full bg-zinc-100 py-1 pl-1 pr-3 ring-1 ring-zinc-200/70 sm:flex">
              <span className="flex h-7 w-7 items-center justify-center rounded-full bg-gradient-to-br from-zinc-600 to-zinc-900 text-[10px] font-bold text-white ring-1 ring-white/20">MK</span>
