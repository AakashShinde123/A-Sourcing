...
| "Sync failed — operations kept in queue" after taking photos | old build vs new DB schema mismatch | this build **self-migrates**: the first photo sync adds the missing column automatically. If you still see it, run `prisma/neon-evidence-image.sql` and pull-to-refresh the phone app — queued operations then sync on their own |
| Reports show nothing to open / no depreciation | reports were version stubs; valuation needs Cost & Purchase Date columns | open any report — the full document (summary, verification breakdown, **Depreciation & Valuation**, exceptions) renders in-app. To value assets, import the register with `Cost` and `Purchase Date` columns (sample CSV in the import dialog shows the format) |
